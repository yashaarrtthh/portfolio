    ...
    <section id="home" className="relative w-full h-screen overflow-hidden bg-black">
      ...
      <div className="hidden lg:flex flex-col gap-6 fixed left-6 top-1/2 -translate-y-1/2 z-50 mix-blend-difference">
        {availableSocials.map((item) => (
          <a
            ...
            className={`text-white/60 ${item.hover} transition-all duration-300 transform hover:scale-125`}
      ...
      <div className="absolute inset-0 z-20 px-6 pb-20 md:pb-[8%] md:px-12 max-w-7xl mx-auto flex flex-col md:flex-row justify-end md:justify-between items-start md:items-end text-left w-full">
        ...
        <div className="flex flex-col items-start text-left max-w-2xl w-full">
          ...
          >
            {availableSocials.map((item) => (
              <a key={item.key} href={item.href} target="_blank" rel="noopener noreferrer" className={`text-white/60 ${item.hover}`} aria-label={item.label}>
                {item.icon}
              </a>
            ))}
          </div>
